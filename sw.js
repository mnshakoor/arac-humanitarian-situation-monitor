const CACHE='ahsm-shell-v081-r1';
...
function annotate(response,source){
  try{
    const headers=new Headers(response.headers);headers.set('X-AHSM-Source',source);
...
async function networkFirst(req){
  ...
  try{
    ...
    return annotate(fresh,'network');
  }catch(error){
    const cached=await caches.match(req,{ignoreSearch:true});
    if(cached)return annotate(cached,'offline-cache');
    if(req.mode==='navigate'){
      const shell=await caches.match('./index.html',{ignoreSearch:true});
      if(shell)return annotate(shell,'offline-cache');
    }
    throw error;
  }
}
...
async function cacheFirst(req){
  const cached=await caches.match(req,{ignoreSearch:true});
  if(cached)return annotate(cached,'cache');
