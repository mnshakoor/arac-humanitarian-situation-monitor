const BUILD_HEALTH_STYLE=`
.status{min-width:300px;max-width:390px;padding:16px 18px}
.status-row{display:grid!important;grid-template-columns:minmax(92px,.72fr) minmax(0,1.28fr);gap:16px;align-items:start;margin:9px 0!important;line-height:1.35}
.status-row>span:first-child{color:var(--muted);white-space:nowrap}
.status-row>span:last-child,.status-row>strong:last-child{min-width:0;text-align:right;overflow-wrap:anywhere;word-break:normal}
.status-row strong{letter-spacing:.015em}
.build-health{margin-top:14px;padding-top:14px;border-top:1px solid var(--line)}
