...
const current7Body = {
  ...
  filter:{operator:'AND',conditions:[
    {field:'status',value:'published'},
    {field:'date.original',value:{from:reliefWebIso(daysAgo(7)),to:reliefWebIso(now)}}
  ]},
  facets:[{name:'countries',field:'primary_country.iso3',limit:250,sort:'count:desc'}]
};
...
const previous7Body = {
  ...
  filter:{operator:'AND',conditions:[
    {field:'status',value:'published'},
    {field:'date.original',value:{from:reliefWebIso(daysAgo(14)),to:reliefWebIso(daysAgo(7))}}
  ]},
  facets:[{name:'countries',field:'primary_country.iso3',limit:250,sort:'count:desc'}]
};

const countryCatalogBody = {
  limit:300,
  fields:{include:['name','shortname','iso3','location','profile.overview','url']}
};

const disasterBody = {
  limit:150,
  filter:{field:'status',value:['ongoing','alert'],operator:'OR'},
  fields:{include:['name','date.event','status','glide','country','primary_country','primary_type','type','url']}
};
...
const [global, current7, previous7, disasters, catalogResponse] = await Promise.all([
  ...
  post('countries', countryCatalogBody)
]);
...
const m7 = countByValue(c7);
const mp7 = countByValue(p7);
...
const countries = c30.map(x => {
  ...
  const reports7d = m7.get(iso3) || 0;
  const previous7d = mp7.get(iso3) || 0;
  const change7d = previous7d > 0 ? ((reports7d - previous7d) / previous7d) * 100 : null;
  return {
    name:meta.name || iso3.toUpperCase(),
    shortname:meta.shortname || meta.name || iso3.toUpperCase(),
    iso3,
...
function normalizeReport(item) {
  ...
  return {
    id:item.id,
    title:f.title,
    dateOriginal:f.date?.original || f['date.original'],
...
const profileResponses = await Promise.all(profileTargets.map(c => post('reports', {
  ...
  filter:{operator:'AND',conditions:[
    {field:'status',value:'published'},
    {field:'primary_country.iso3',value:c.iso3},
    {field:'date.original',value:{from:reliefWebIso(daysAgo(30)),to:reliefWebIso(now)}}
  ]},
  fields:{include:reportFields},
  facets:[
...
const normalizedDisasters = (disasters.data || []).map(d => {
  ...
  return {
    ...
    status:f.status,
    glide:f.glide,
    dateEvent:f.date?.event,
    url:f.url || d.href,
    primaryCountry:f.primary_country?.name || '',
    primaryCountryIso3:String(f.primary_country?.iso3 || '').toLowerCase(),
    location:f.primary_country?.location || null,
    primaryType:f.primary_type?.name || '',
    types:(f.type || []).map(x => x.name)
  };
});
...
const snapshot = {
  ...
  status:'RELIEFWEB LIVE SNAPSHOT',
  stale:false,
  summary:{
    reports30d:global.totalCount ?? global.total_count ?? reports.length,
    countries:countries.length,
    uniqueSources:facetMap(global,'sources').length,
    activeDisasters:disasters.totalCount ?? disasters.total_count ?? normalizedDisasters.length,
    profiledCountries:profileTargets.length
  },
  timeline:facetMap(global,'timeline').map(x => ({date:facetValue(x),count:facetCount(x)})),
  globalThemes:normalizeFacet(facetMap(global,'themes')),
  globalSources:normalizeFacet(facetMap(global,'sources')),
  globalFormats:normalizeFacet(facetMap(global,'formats')),
  countries,
  reports,
  disasters:normalizedDisasters,
  provenance:{
    provider:'ReliefWeb API V2',
    appname,
    queryGeneratedAt:now.toISOString(),
    dateBasis:'date.original',
    status:'published',
    countryCounting:'primary_country.iso3',
    countryProfileLimit:PROFILE_LIMIT,
    analyticalBoundary:'Reporting intensity and momentum are information-environment signals, not humanitarian severity measures.'
  }
};

await fs.mkdir('data',{recursive:true});
const target='data/snapshot.json';
const tmp='data/snapshot.next.json';
