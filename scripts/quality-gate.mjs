...
for(const marker of ['PARTIAL','DEGRADED','progressive-controls','Related reports (0 in snapshot)','methodology-public-grid','rc-mobile-collapsed','primary_country.iso3','max-height:650px','age>30'])if(!v091.includes(marker))throw new Error(`RC1 field-test refinement layer missing ${marker}`);
const sync=fs.readFileSync('scripts/sync-reliefweb.mjs','utf8');
...
for(const marker of ['ahsm-shell-v090-rc1-r3','X-AHSM-Source','offline-cache','ignoreSearch:true','./js/v081.js','./js/v083.js','./js/v09.js','./js/v091.js'])if(!sw.includes(marker))throw new Error(`Service-worker RC1 diagnostics missing ${marker}`);
