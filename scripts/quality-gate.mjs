...
for(const marker of ['PARTIAL','DEGRADED','progressive-controls','Related reports (0 in snapshot)','methodology-public-grid','rc-mobile-collapsed','primary_country.iso3','responsivePageSize','orientationchange','age>30'])if(!v091.includes(marker))throw new Error(`RC1 field-test refinement layer missing ${marker}`);
const v092=fs.readFileSync('js/v092.js','utf8');
for(const marker of ['network-secondary-analysis','NETWORK INTERPRETATION','TOPIC SIGNALS','secondary-analysis-band','network-intelligence-panel','order:6'])if(!v092.includes(marker))throw new Error(`RC1.1 UI refinement layer missing ${marker}`);
