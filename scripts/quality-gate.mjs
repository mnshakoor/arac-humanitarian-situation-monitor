...
for(const layer of ["import './v04.js'","import './v05.js'","import './v06.js'","import './v07.js'","import './v08.js'","import './v081.js'","import './v083.js'","import './v07-style.js'","import './v08-style.js'","import './region-map.js'"])if(!ui.includes(layer))throw new Error(`UI layer missing ${layer}`);
const config=fs.readFileSync('js/config.js','utf8');
if(!config.includes("0.8.3-beta"))throw new Error('Build version is not v0.8.3-beta');
...
for(const marker of ['network-analysis-flow','graph-first','temporal-intelligence','temporal-investigation','investigation-tray','nth-child(2){order:1'])if(!v083.includes(marker))throw new Error(`v0.8.3 Network Explorer layout layer missing ${marker}`);
const sw=fs.readFileSync('sw.js','utf8');
for(const marker of ['ahsm-shell-v083-r1','X-AHSM-Source','offline-cache','ignoreSearch:true','./js/v081.js','./js/v083.js'])if(!sw.includes(marker))throw new Error(`Service-worker diagnostics missing ${marker}`);
...
console.log(`AHSM v0.8.3 quality gate passed: ${snapshot.countries.length} countries, ${snapshot.reports.length} core reports, ${snapshot.disasters.length} disaster contexts.`);
