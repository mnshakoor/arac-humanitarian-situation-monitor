...
if(!config.includes("0.8.1-beta"))throw new Error('Build version is not v0.8.1-beta');
...
for(const marker of ['Build Health / Update Status','Displayed build','Deployed build','Service-worker cache','Verification source','updateViaCache'])if(!v081.includes(marker))throw new Error(`v0.8.1 build-health layer missing ${marker}`);
const sw=fs.readFileSync('sw.js','utf8');
for(const marker of ['ahsm-shell-v081-r1','X-AHSM-Source','offline-cache','ignoreSearch:true','./js/v081.js'])if(!sw.includes(marker))throw new Error(`Service-worker diagnostics missing ${marker}`);
...
console.log(`AHSM v0.8.1 quality gate passed: ${snapshot.countries.length} countries, ${snapshot.reports.length} core reports, ${snapshot.disasters.length} disaster contexts.`);
