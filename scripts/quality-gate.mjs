...
const required=['index.html','manifest.webmanifest','sw.js','css/app.css','css/ui-patch.css','css/v04.css','css/v05.css','css/v06.css','css/v07.css','js/app.js','js/ui-patch.js','js/v03.js','js/v04.js','js/v05.js','js/v06.js','js/v07.js','js/v07-style.js','js/region-map.js','js/config.js','data/snapshot.json'];
...
for(const layer of ["import './v04.js'","import './v05.js'","import './v06.js'","import './v07.js'","import './v07-style.js'","import './region-map.js'"])if(!ui.includes(layer))throw new Error(`UI layer missing ${layer}`);
const config=fs.readFileSync('js/config.js','utf8');
if(!config.includes("0.7.0-beta"))throw new Error('Build version is not v0.7.0-beta');
...
console.log(`AHSM v0.7 quality gate passed: ${snapshot.countries.length} countries, ${snapshot.reports.length} core reports, ${snapshot.disasters.length} disaster contexts.`);
