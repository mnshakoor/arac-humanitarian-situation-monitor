...
const required=['index.html','manifest.webmanifest','sw.js','css/app.css','css/ui-patch.css','css/v04.css','css/v05.css','css/v06.css','js/app.js','js/ui-patch.js','js/v03.js','js/v04.js','js/v05.js','js/v06.js','js/region-map.js','js/config.js','data/snapshot.json'];
...
for(const layer of ["import './v04.js'","import './v05.js'","import './v06.js'","import './region-map.js'"])if(!ui.includes(layer))throw new Error(`UI layer missing ${layer}`);
const config=fs.readFileSync('js/config.js','utf8');
if(!config.includes("0.6.0-beta"))throw new Error('Build version is not v0.6.0-beta');
...
console.log(`AHSM v0.6 quality gate passed: ${snapshot.countries.length} countries, ${snapshot.reports.length} core reports, ${snapshot.disasters.length} disaster contexts.`);
